    ...
    return (
        <AuthenticatedLayout>
            ...
            <Card className="mb-6 flex flex-col items-center p-6 text-center">
                ...
                <p className="mt-3 text-lg font-bold text-slate-900">{auth.user?.name}</p>
                <p className="text-sm text-slate-500 capitalize">{auth.user?.roles?.join(', ') || 'Pengguna'}</p>
            </Card>

            <h2 className="mb-3 px-1 text-xs font-semibold uppercase tracking-wide text-slate-400">Template Pengingat</h2>

            <Card className="mb-5 p-4 text-sm text-slate-600">
                <span className="flex items-center gap-2 font-medium text-slate-700"><MessageSquareText size={16} /> Variabel tersedia</span>
                <div className="mt-2">
            ...
            <button
                ...
                className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-600 transition hover:bg-rose-100"
            >

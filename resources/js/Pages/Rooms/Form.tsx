                ...
                <div className="flex flex-wrap gap-2">
                    {FACILITY_OPTIONS.map((f) => (
                        <button
                            ...
                            key={f}
                            onClick={() => toggle(f)}
                            className={`rounded-full border px-3 py-1 text-sm transition ${selected.includes(f) ? 'border-brand-500 bg-brand-50 text-brand-700' : 'border-slate-300 text-slate-600'}`}
                        >
                            {f}
                        </button>
                    ))}

        ...
        <AuthenticatedLayout>
            ...
            <div className="grid gap-6 lg:grid-cols-3">
                <Card className="p-6 lg:col-span-2">
                    <h2 className="mb-3 text-sm font-semibold text-slate-700 dark:text-slate-200">Rincian</h2>
                    <table className="w-full text-sm">
                        ...
                        <tfoot>
                            <tr className="border-t border-slate-200 font-semibold dark:border-slate-700"><td className="py-2" colSpan={2}>Total</td><td className="py-2 text-right">{rupiah(invoice.amount)}</td></tr>
                ...
                <Card className="p-6">
                    <dl className="space-y-2 text-sm">
                        <div className="flex justify-between"><dt className="text-slate-500 dark:text-slate-400">Status</dt><dd><Badge status={invoice.status} label={invoice.status} /></dd></div>
                        <div className="flex justify-between"><dt className="text-slate-500 dark:text-slate-400">Periode</dt><dd>{tanggal(invoice.period_start)} – {tanggal(invoice.period_end)}</dd></div>
                        <div className="flex justify-between"><dt className="text-slate-500 dark:text-slate-400">Jatuh Tempo</dt><dd>{tanggal(invoice.due_date)}</dd></div>
                    </dl>
                    <h3 className="mb-2 mt-5 text-sm font-semibold text-slate-700 dark:text-slate-200">Riwayat Pembayaran</h3>
                    <div className="space-y-2">
                        {(invoice.payments ?? []).map((p) => (
                            <div key={p.id} className="rounded-lg border border-slate-100 px-3 py-2 text-sm dark:border-slate-800">
